...
class PPO(object):
    ...
    def learn(self):
        ...
        d_batch = torch.FloatTensor(self.done)
        state_next = torch.FloatTensor(self.state_next)

        s_next_batch = torch.cat((s_batch[1:, :], torch.unsqueeze(state_next, dim=0)), dim=0)

        v_target = r_batch+(self.conf.discount*(1-d_batch)*self.critic(s_next_batch).squeeze()).detach()
...
if __name__ == '__main__':
    ...
    for episode in range(conf.episodes):
        ...
        while not done and step < conf.step_max:
            ...
            ppo.store_transition(obs, action, reward, obs_next, done)
